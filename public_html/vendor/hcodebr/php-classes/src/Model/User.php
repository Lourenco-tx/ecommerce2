	...
	public function delete()
	{
	 ...
	 public static function getForgot($email){
		...
		if (count($results) === 0) {
			...
			$resultsRecovery = $sql->select("CALL sp_userspasswordsrecoveries_create(:iduser, :desip)", array(":iduser"=>$data["iduser"], ":desip"=>$_SERVER["REMOTE_ADDR"]));
			...
			if (count($resultsRecovery) === 0) {
				...
				$link = "http://www.personflix.esy.es/admin/forgot/reset?code=$code";
				$mailer = new Mailer ($data["desemail"], $data["desperson"], "Redifinir Senha da Hcode Store", "forgot", array("name"=>$data["desperson"], "link"=>$link));
				$mailer->send();
				return $data;
	        }
	    }
...
>= NOW();", array(":idrecovery"=>$idrecovery));
        if (count($results) === 0)
        {
            throw new \Exception("Não foi possível recuperar a senha.");
        }
